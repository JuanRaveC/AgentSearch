...
if __name__ == '__main__':
    ...
    join_agent = threading.Thread(target=JoinAgent(join_agent_queue))
    join_agent.daemon = True
    join_agent.start()

    #agente procesador
    process_agent_queue = Queue() 
    process_agent = threading.Thread(target=ProcessAgent(process_agent_queue))
    process_agent.daemon = True
    process_agent.start()

    #agente indexador
    index_agent_queue = Queue()
    index_agent = threading.Thread(target=IndexAgent(index_agent_queue))
